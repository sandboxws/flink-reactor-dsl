...
import { COMPONENT_REGISTRY, collectPeerDeps, STYLE_FILES } from "../config.js"
...
export async function init(
  ...
  fs.mkdirSync(path.join(targetDir, "src/components/layout"), {
    recursive: true,
  })
  fs.mkdirSync(path.join(targetDir, "src/components/shared"), {
    recursive: true,
  })
  ...
  const _peerDeps = collectPeerDeps(allComponents)
  const packageJson = {
    name: name,
    version: "0.1.0",
    private: true,
