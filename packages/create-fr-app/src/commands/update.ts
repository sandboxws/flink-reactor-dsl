...
export async function update(options: UpdateOptions) {
  ...
  for (const [compName, info] of Object.entries(installed)) {
    ...
    const hasLocalModification = fs.existsSync(localPath) &&
  ...
  for (const { name, hasConflict } of updates) {
    if (hasConflict) {
      console.log(`  ${pc.yellow("⚠")} ${pc.cyan(name)} - has local modifications`);
    } else {
  ...
  if (conflicts.length > 0 && !options.force) {
    ...
    if (!options.yes) {
      ...
      if (choice === "skip") {
        ...
        updates.splice(0, updates.length, ...updates.filter((u) => !conflictNames.has(u.name)));
      }
    }
  }
