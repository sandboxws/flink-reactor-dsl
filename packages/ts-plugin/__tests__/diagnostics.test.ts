...
describe("getNestingDiagnostics", () => {
  ...
  it("reports warning for KafkaSink inside Route", () => {
    ...
    expect(diags).toHaveLength(1)
    expect(diags[0].messageText).toContain(
      "'KafkaSink' is not a valid child of 'Route'",
    )
  })
  ...
  it("checks nested structures independently", () => {
    ...
    const sf = parse("<Pipeline><Filter /><Route><Filter /></Route></Pipeline>")
    const diags = getNestingDiagnostics(sf, registry, ts)
    expect(diags).toHaveLength(1)
    expect(diags[0].messageText).toContain(
      "'Filter' is not a valid child of 'Route'",
  ...
  it("validates Query children", () => {
    const sf = parse("<Query><Query.Select></Query.Select><Filter /></Query>")
    const diags = getNestingDiagnostics(sf, registry, ts)
    expect(diags).toHaveLength(1)
    expect(diags[0].messageText).toContain(
      "'Filter' is not a valid child of 'Query'",
    )
  })
})
