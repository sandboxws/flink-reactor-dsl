 ...
 * ```
 */
export function defineConfig(config: Omit<FlinkReactorConfig, 'toInfraConfig'>): FlinkReactorConfig {
  // Validate flink version if provided
  if (config.flink?.version !== undefined) {
    const validVersions: readonly FlinkMajorVersion[] = ['1.20', '2.0', '2.1', '2.2'];
  ...
  if (config.environments) {
    ...
    for (const [envName, envEntry] of Object.entries(config.environments)) {
      if (envEntry.dashboard?.auth?.type === 'basic') {
        if (!envEntry.dashboard.auth.username || !envEntry.dashboard.auth.password) {
          throw new Error(
