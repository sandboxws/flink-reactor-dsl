...
import { KafkaSource, JdbcSource, GenericSource } from '../../components/sources.js';
import { KafkaSink, JdbcSink, FileSystemSink } from '../../components/sinks.js';
import { Filter, Map as MapTransform } from '../../components/transforms.js';
...
import { generateTapManifest } from '../../codegen/sql-generator.js';
...
describe('resolveObservationStrategy', () => {
  ...
  it('maps upsert-kafka to consumer-group-clone', () => {
    expect(resolveObservationStrategy('upsert-kafka')).toBe('consumer-group-clone');
  });
...
describe('Kafka source tap observation SQL', () => {
  it('generates correct observation SQL with consumer group clone', () => {
    const schema = { order_id: 'BIGINT', user_id: 'STRING', amount: 'DECIMAL(10, 2)' };
...
describe('generateTapMetadata - dev mode', () => {
  it('auto-taps all sinks but not untapped transforms', () => {
    ...
    const { taps, diagnostics } = generateTapMetadata(pipeline, 'ecommerce', '2.0', true);
...
describe('validateTapConfig', () => {
  ...
  it('validates endTimestamp must be after startTimestamp', () => {
    ...
    const diagnostics = validateTapConfig(config, 'orders');
    expect(diagnostics).toHaveLength(1);
    expect(diagnostics[0].message).toContain('endTimestamp must be after startTimestamp');
  });
...
describe('Full pipeline TapManifest', () => {
  it('generates correct manifest JSON for mixed tapped/untapped operators', () => {
    ...
    const sourceTap = manifest!.taps.find((t) => t.componentName === 'KafkaSource');
...
describe('FileSystem sink tap unsupported', () => {
  it('emits unsupported warning diagnostic', () => {
    ...
    const { taps, diagnostics } = generateTapMetadata(pipeline, 'test', '2.0', false);
