...
import { synthesizeApp, type FlinkReactorAppProps } from '../app.js';
import { resolveEnvironment, discoverEnvironments, type EnvironmentConfig } from '../environment.js';
...
function buildOrderPipeline(opts?: { parallelism?: number; bootstrapServers?: string }) {
...
describe('5.6: validate() test helper', () => {
  ...
  it('detects orphan source (source with no downstream)', () => {
    ...
    expect(result.errors.some((e) => e.message.includes('Orphan source'))).toBe(true);
  });

  it('detects more orphan sources in larger pipelines', () => {
    // Two orphan sources + one connected
    const orphan1 = KafkaSource({ topic: 'orphan1', format: 'json', schema: EventSchema });
    const orphan2 = KafkaSource({ topic: 'orphan2', format: 'json', schema: EventSchema });
    const connected = KafkaSource({ topic: 'connected', format: 'json', schema: EventSchema });

    const sink = KafkaSink({ topic: 'out', children: [connected] });
    const pipeline = Pipeline({ name: 'multi-orphan', children: [sink, orphan1, orphan2] });
    ...
    const orphanErrors = result.errors.filter((e) => e.message.includes('Orphan source'));
