...
import { createElement, resetNodeIdCounter, resetComponentKinds } from '../jsx-runtime.js';
...
function buildTestPipeline(): ConstructNode {
  return createElement('Pipeline', { name: 'test-pipeline' },
...
describe('plugin integration with synthesizeApp', () => {
  ...
  it('applies CRD transformer plugin', () => {
    const labelsPlugin: FlinkReactorPlugin = {
      ...
      transformCrd: (crd) => ({
        ...
        metadata: {
          ...crd.metadata,
          labels: { ...crd.metadata.labels, 'app.kubernetes.io/managed-by': 'flink-reactor' },
        },
      }),
    };
  ...
  it('respects plugin ordering in tree transformers', () => {
    ...
    const pluginA: FlinkReactorPlugin = {
      name: 'alpha',
      transformTree: (tree) => { log.push('alpha'); return tree; },
    };

    const pluginB: FlinkReactorPlugin = {
      name: 'beta',
      ordering: { after: ['alpha'] },
      transformTree: (tree) => { log.push('beta'); return tree; },
    };
  ...
  it('merges plugins from config and options', () => {
    ...
    const configPlugin: FlinkReactorPlugin = {
      name: 'from-config',
      beforeSynth: () => { log.push('config'); },
    };

    const optionsPlugin: FlinkReactorPlugin = {
      name: 'from-options',
      beforeSynth: () => { log.push('options'); },
    };
  ...
  it('registers and uses custom component kinds', () => {
    const customPlugin: FlinkReactorPlugin = {
      ...
      ddlGenerators: new Map([
        ['RedisSource', (node) => `CREATE TABLE \`${node.id}\` WITH (\n  'connector' = 'redis'\n);`],
      ]),
    };

    // Build pipeline using the custom component
    const pipeline = createElement('Pipeline', { name: 'redis-pipeline' },
      createElement('KafkaSink', { topic: 'output', bootstrapServers: 'kafka:9092' },
...
describe('plugin integration with validate() test helper', () => {
  ...
  it('validate() works without plugins (backward compatible)', () => {
    ...
    const pipeline = createElement('Pipeline', { name: 'valid' },
      createElement('KafkaSource', { topic: 'in', schema: testSchema },
        createElement('Filter', { condition: 'amount > 0' },
          createElement('KafkaSink', { topic: 'out', bootstrapServers: 'kafka:9092' }),
        ),
      ),
    );
...
describe('plugin error handling', () => {
  it('cleans up component registrations even when synthesis throws', () => {
    const failPlugin: FlinkReactorPlugin = {
      ...
      transformTree: () => { throw new Error('intentional failure'); },
    };

    const pipeline = buildTestPipeline();

    expect(() => synthesizeApp(
      { name: 'test-app', children: pipeline },
      { plugins: [failPlugin] },
