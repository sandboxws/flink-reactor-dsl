 ...
 */
export function inferExpressionType(expr: string, upstreamFields: Map<string, string>): string {
 ...
 */
export function resolveNodeSchema(node: ConstructNode, nodeIndex: Map<string, ConstructNode>): ResolvedColumn[] | null {
 ...
 */
export function findRouteUpstreamNode(routeNode: ConstructNode, parent?: ConstructNode): ConstructNode | null {
...
export function indexTree(node: ConstructNode, index: Map<string, ConstructNode>): void {
 ...
 */
export function introspectPipelineSchemas(pipelineNode: ConstructNode): IntrospectedSchema[] {
  ...
  for (const src of sources) {
    ...
    const propsKeySet = new Set((src.props.primaryKey as readonly string[] | undefined) ?? []);
    const wmCol = schemaDef.watermark?.column;
    const metaCols = new Map(
    ...
    for (const meta of schemaDef.metadataColumns) {
      const constraints: ConstraintKind[] = [meta.isVirtual ? 'META VIRTUAL' : 'META'];
      columns.push({ name: meta.column, type: meta.type as string, constraints });
    }
  ...
  for (const sink of sinks) {
    ...
    schemas.push({
      ...
      kind: 'sink',
      nameHint: (sink.props._nameHint as string) ?? sink.id,
      columns: meta.map((c) => ({ name: c.name, type: c.type, constraints: [] })),
    });
  }
