...
describe('detectJdbcDialect', () => {
  ...
  it('detects SQL Server from URL', () => {
    const dialect = detectJdbcDialect('jdbc:sqlserver://localhost:1433;database=mydb');
    expect(dialect).toBeDefined();
...
describe('resolveJdbcDialectArtifacts', () => {
  it('resolves PostgreSQL: core + dialect + driver for Flink 2.0+', () => {
    const artifacts = resolveJdbcDialectArtifacts('jdbc:postgresql://localhost:5432/mydb', '2.0');
  ...
  it('resolves only driver for Flink 1.20 (single JAR includes dialects)', () => {
    const artifacts = resolveJdbcDialectArtifacts('jdbc:postgresql://localhost:5432/mydb', '1.20');
    expect(artifacts).toHaveLength(1);
    expect(artifacts[0].artifactId).toBe('postgresql');
  });

  it('returns empty for unknown dialect URL', () => {
    expect(resolveJdbcDialectArtifacts('jdbc:h2:mem:test', '2.0')).toHaveLength(0);
  });
});
...
describe('artifactToMavenUrl', () => {
  ...
  it('uses custom base URL', () => {
    ...
    expect(url).toContain('https://nexus.internal/maven-central/org/apache/flink');
  });
});

describe('artifactToJarName', () => {
  it('generates correct JAR filename', () => {
    expect(artifactToJarName({
      groupId: 'org.apache.flink',
      artifactId: 'flink-sql-connector-kafka',
      version: '4.0.1-2.0',
