  ...
  return [
    {
      ...
      downloadUrl: 'https://repo1.maven.org/maven2/org/apache/flink/flink-sql-connector-kafka/4.0.1-2.0/flink-sql-connector-kafka-4.0.1-2.0.jar',
    ...
    },
    {
      ...
      downloadUrl: 'https://repo1.maven.org/maven2/org/apache/flink/flink-sql-avro/1.20.0/flink-sql-avro-1.20.0.jar',
      provenance: ['KafkaSource_0'],
    },
  ];
}
...
describe('init-container delivery', () => {
  it('generates correct initContainers YAML structure', () => {
    ...
    expect(cmd[1]).toBe('-c');
    expect(cmd[2]).toContain('wget -O /opt/flink/usrlib/flink-sql-connector-kafka-4.0.1-2.0.jar');
    expect(cmd[2]).toContain('wget -O /opt/flink/usrlib/flink-sql-avro-1.20.0.jar');
  ...
  it('generates pipeline.classpaths entries', () => {
    ...
    expect(patch.classpaths[0]).toBe('file:///opt/flink/usrlib/flink-sql-connector-kafka-4.0.1-2.0.jar');
    expect(patch.classpaths[1]).toBe('file:///opt/flink/usrlib/flink-sql-avro-1.20.0.jar');
  });
...
describe('custom-image delivery', () => {
  it('generates valid Dockerfile', () => {
    ...
    expect(result.dockerfile).toContain('ADD https://repo1.maven.org/maven2/org/apache/flink/flink-sql-connector-kafka/4.0.1-2.0/flink-sql-connector-kafka-4.0.1-2.0.jar /opt/flink/usrlib/flink-sql-connector-kafka-4.0.1-2.0.jar');
    expect(result.dockerfile).toContain('ADD https://repo1.maven.org/maven2/org/apache/flink/flink-sql-avro/1.20.0/flink-sql-avro-1.20.0.jar /opt/flink/usrlib/flink-sql-avro-1.20.0.jar');
    expect(result.dockerfile).toContain('RUN chmod -R 644 /opt/flink/usrlib/*.jar');
  });
...
describe('air-gapped: Maven mirror substitution', () => {
  it('substitutes Maven Central URL with mirror', () => {
    const jars = makeTestJars();
    const mirrored = applyMavenMirror(jars, 'https://nexus.internal/maven-central');
...
describe('air-gapped: private registry', () => {
  ...
  it('adds imagePullSecrets when provided', () => {
    const pipeline = Pipeline({ name: 'test', children: [] });
    const crd = generateCrd(pipeline, { flinkVersion: '2.0' });
    const patched = applyPrivateRegistry(crd, 'registry.internal.com', 'my-pull-secret');
...
describe('end-to-end: resolve + init-container delivery', () => {
  it('resolves connectors and applies init-container patch', () => {
    ...
    expect(finalCrd.spec.flinkConfiguration['pipeline.classpaths']).toBeDefined();
