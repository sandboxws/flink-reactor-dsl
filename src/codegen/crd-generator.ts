...
const DURATION_REGEX = /^(\d+)\s*(ms|s|sec|second|seconds|m|min|minute|minutes|h|hour|hours|d|day|days)$/i;
...
export function generateCrd(
  ...
  if (props.checkpoint) {
    config['execution.checkpointing.interval'] = String(toMilliseconds(props.checkpoint.interval));
    if (props.checkpoint.mode) {
      config['execution.checkpointing.mode'] = props.checkpoint.mode === 'exactly-once'
        ? 'EXACTLY_ONCE'
        : 'AT_LEAST_ONCE';
    }
  }
  ...
  if (props.restartStrategy) {
    ...
    if (props.restartStrategy.type === 'fixed-delay') {
      if (props.restartStrategy.attempts !== undefined) {
        config['restart-strategy.fixed-delay.attempts'] = String(props.restartStrategy.attempts);
      }
      if (props.restartStrategy.delay) {
        config['restart-strategy.fixed-delay.delay'] = props.restartStrategy.delay;
      }
    }
  }
  ...
  const normalizedConfig = FlinkVersionCompat.normalizeConfig(config, flinkVersion);
