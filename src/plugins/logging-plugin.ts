...
import { mapTree, findNodes } from '../core/tree-utils.js';
...
export interface LoggingPluginOptions {
   ...
   */
  readonly loggers?: Readonly<Record<string, 'TRACE' | 'DEBUG' | 'INFO' | 'WARN' | 'ERROR'>>;
  /**
...
const DEFAULT_TARGETS: readonly ConstructNode['kind'][] = ['Source', 'Sink', 'Transform'];
 ...
 * ```
 */
export function loggingPlugin(options: LoggingPluginOptions = {}): FlinkReactorPlugin {
  ...
  return {
    ...
    transformCrd(crd, pipelineNode) {
      ...
      const annotatedCount = findNodes(pipelineNode, (n) =>
        n.props._logging != null,
      ).length;
