  ...
  return {
    ...
    transformCrd(crd) {
      ...
      switch (strategy.type) {
        case 'fixed-delay': {
          config['restart-strategy.fixed-delay.attempts'] = String(strategy.attempts ?? 3);
        ...
        case 'failure-rate': {
          ...
          config['restart-strategy.failure-rate.delay'] = strategy.delay ?? '10s';
          break;
        }
        case 'exponential-delay': {
          ...
          config['restart-strategy.exponential-delay.jitter-factor'] =
            String(strategy.jitter ?? 0.1);
          break;
        }
      }
