...
import { createElement, resetNodeIdCounter, resetComponentKinds } from '../../core/jsx-runtime.js';
...
function buildUnprotectedPipeline(): ConstructNode {
  return createElement('Pipeline', { name: 'unprotected' },
    createElement('KafkaSink', { topic: 'output', bootstrapServers: 'kafka:9092' },
      createElement('Filter', { condition: 'amount > 0' },
...
function buildProtectedPipeline(): ConstructNode {
  return createElement('Pipeline', { name: 'protected' },
    createElement('KafkaSink', { topic: 'output', bootstrapServers: 'kafka:9092' },
...
describe('errorHandlingPlugin', () => {
  ...
  describe('CRD transformer — failure-rate', () => {
    it('configures failure-rate strategy with defaults', () => {
      ...
      expect(config['restart-strategy.failure-rate.max-failures-per-interval']).toBe('3');
      expect(config['restart-strategy.failure-rate.failure-rate-interval']).toBe('5m');
    ...
    it('configures failure-rate strategy with custom values', () => {
      ...
      expect(config['restart-strategy.failure-rate.max-failures-per-interval']).toBe('5');
      expect(config['restart-strategy.failure-rate.failure-rate-interval']).toBe('10m');
  ...
  describe('CRD transformer — exponential-delay', () => {
    it('configures exponential-delay strategy with defaults', () => {
      ...
      expect(config['restart-strategy.exponential-delay.initial-backoff']).toBe('1s');
      expect(config['restart-strategy.exponential-delay.max-backoff']).toBe('5m');
      expect(config['restart-strategy.exponential-delay.backoff-multiplier']).toBe('2');
      expect(config['restart-strategy.exponential-delay.reset-backoff-threshold']).toBe('10m');
      expect(config['restart-strategy.exponential-delay.jitter-factor']).toBe('0.1');
    });
    ...
    it('configures exponential-delay with custom values', () => {
      ...
      expect(config['restart-strategy.exponential-delay.initial-backoff']).toBe('2s');
      expect(config['restart-strategy.exponential-delay.max-backoff']).toBe('10m');
      expect(config['restart-strategy.exponential-delay.backoff-multiplier']).toBe('3');
      expect(config['restart-strategy.exponential-delay.reset-backoff-threshold']).toBe('30m');
      expect(config['restart-strategy.exponential-delay.jitter-factor']).toBe('0.2');
    });
  });
  ...
  describe('metadata annotations', () => {
    it('adds restart strategy annotation', () => {
      ...
      const result = synth(pipeline, { plugins: [plugin] });
      expect(result.crd.metadata.annotations!['flink-reactor.io/restart-strategy']).toBe(
        'failure-rate',
      );
    });
  });
  ...
  describe('integration', () => {
    it('works through synthesizeApp', () => {
      ...
      expect(result.pipelines[0].crd.spec.flinkConfiguration['restart-strategy.type']).toBe(
        'fixed-delay',
      );
    });
