...
function buildPipeline(): ConstructNode {
  return createElement('Pipeline', { name: 'metrics-test' },
...
describe('metricsPlugin', () => {
  ...
  describe('Prometheus reporter', () => {
    ...
    it('configures Prometheus with custom port', () => {
      ...
      const result = synth(pipeline, { plugins: [plugin] });
      expect(result.crd.spec.flinkConfiguration['metrics.reporter.prometheus.port']).toBe('9999');
    });
  });
  ...
  describe('SLF4J reporter', () => {
    ...
    it('configures SLF4J with custom interval', () => {
      ...
      const result = synth(pipeline, { plugins: [plugin] });
      expect(result.crd.spec.flinkConfiguration['metrics.reporter.slf4j.interval']).toBe('30s');
    });
  });
  ...
  describe('multiple reporters', () => {
    it('configures multiple reporters simultaneously', () => {
      ...
      expect(result.crd.metadata.annotations!['flink-reactor.io/metric-reporters']).toBe(
        'prometheus,slf4j,jmx',
      );
    });
  });
  ...
  describe('optional settings', () => {
    it('configures scope delimiter', () => {
      ...
      const result = synth(pipeline, { plugins: [plugin] });
      expect(result.crd.spec.flinkConfiguration['metrics.scope.delimiter']).toBe('_');
    });
    ...
    it('configures latency tracking interval', () => {
      ...
      const result = synth(pipeline, { plugins: [plugin] });
      expect(result.crd.spec.flinkConfiguration['metrics.latency.interval']).toBe('5000');
    });
    ...
    it('disables latency tracking when set to 0', () => {
      ...
      const result = synth(pipeline, { plugins: [plugin] });
      expect(result.crd.spec.flinkConfiguration['metrics.latency.interval']).toBe('0');
    });
  });
