...
function buildPipeline(): ConstructNode {
  return createElement('Pipeline', { name: 'log-test' },
...
describe('loggingPlugin', () => {
  ...
  describe('CRD transformer', () => {
    it('adds root logger level to flinkConfiguration', () => {
      ...
      expect(result.crd.spec.flinkConfiguration['rootLogger.level']).toBe('WARN');
    });
    ...
    it('configures additional loggers', () => {
      ...
      expect(config['logger.org_apache_flink_streaming.name']).toBe('org.apache.flink.streaming');
    ...
    it('adds logging metadata annotations', () => {
      ...
      expect(result.crd.metadata.annotations!['flink-reactor.io/logging-tag']).toBe('my-tag');
      expect(result.crd.metadata.annotations!['flink-reactor.io/logging-targets']).toBeDefined();
    });
  });
  ...
  describe('integration', () => {
    it('works through synthesizeApp', () => {
      ...
      expect(result.pipelines[0].crd.spec.flinkConfiguration['rootLogger.level']).toBe('DEBUG');
    });
