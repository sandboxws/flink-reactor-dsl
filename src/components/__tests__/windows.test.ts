...
import { TumbleWindow, SlideWindow, SessionWindow } from '../windows.js';
...
describe('TumbleWindow', () => {
  ...
  it('builds a valid DAG with source → window → aggregate → sink', () => {
    ...
    expect(sorted.map((n) => n.component)).toEqual([
      'KafkaSource', 'TumbleWindow', 'Aggregate', 'KafkaSink',
    ]);
  ...
  it('wraps Aggregate child for session window aggregation', () => {
    const aggregate = Aggregate({
      ...
      select: {
        userId: 'userId',
        sessionPages: 'COUNT(*)',
        sessionDuration: 'TIMESTAMPDIFF(SECOND, MIN(viewTimestamp), MAX(viewTimestamp))',
      },
    });
