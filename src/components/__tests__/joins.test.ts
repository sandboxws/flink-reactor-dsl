...
import { Join, TemporalJoin, LookupJoin, IntervalJoin } from '../joins.js';
...
describe('LookupJoin', () => {
  it('stores async and cache configuration', () => {
    ...
    const join = LookupJoin({
      ...
      select: { product_name: 'dim_products.name', price: 'dim_products.price' },
    ...
    expect(join.props.async).toEqual({ enabled: true, capacity: 100, timeout: '30s' });
