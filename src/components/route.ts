...
function RouteBranch(props: RouteBranchProps): ConstructNode {
  ...
  return createElement('Route.Branch', { ...rest }, ...childArray);
}
...
function RouteDefault(props: RouteDefaultProps): ConstructNode {
  const { children, ...rest } = props;
  const childArray = children == null
    ? []
    : Array.isArray(children)
  ...
  return createElement('Route.Default', { ...rest }, ...childArray);
}
