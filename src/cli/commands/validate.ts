...
export async function runValidate(opts: {
  ...
  console.log(pc.dim(`Validating ${projectCtx.pipelines.length} pipeline(s)...\n`));
  ...
  for (const result of results) {
    if (result.errors.length === 0 && result.warnings.length === 0) {
      console.log(`  ${pc.green('\u2713')} ${pc.bold(result.name)} ${pc.dim('— valid')}`);
    } else {
      if (result.errors.length > 0) {
        console.log(`  ${pc.red('\u2717')} ${pc.bold(result.name)} ${pc.dim(`— ${result.errors.length} error(s)`)}`);
      ...
      if (result.warnings.length > 0) {
        console.log(`  ${pc.yellow('!')} ${pc.bold(result.name)} ${pc.dim(`— ${result.warnings.length} warning(s)`)}`);
...
function validatePipeline(
  ...
  const versionDiagnostics = validateFlinkVersionFeatures(pipelineNode, flinkVersion);
...
function validateFlinkVersionFeatures(
  ...
  function walk(n: ConstructNode): void {
    ...
    const featureChecks: Array<{ component: string; feature: string }> = [
