...
export function registerUpgradeCommand(program: Command): void {
  program
    .command('upgrade')
    .argument('[package]', 'Package to upgrade (e.g. "dashboard" or "@flink-reactor/dashboard")')
...
function getLatestVersion(pkg: string): string | null {
  try {
    return execSync(`npm view ${pkg} version`, { stdio: 'pipe', encoding: 'utf-8' }).trim();
  } catch {
    return null;
  }
}
...
export async function runUpgrade(
  ...
  for (const pkg of packages) {
    ...
    if (!installed) {
      console.log(`  ${pc.green('+')} ${pkg} ${pc.dim('not installed')} → ${pc.green(latest)}`);
      hasUpgrades = true;
    } else if (installed !== latest) {
      console.log(`  ${pc.yellow('↑')} ${pkg} ${pc.dim(installed)} → ${pc.green(latest)}`);
      hasUpgrades = true;
    } else {
      console.log(`  ${pc.green('✓')} ${pkg} ${pc.dim(installed)} ${pc.dim('(up to date)')}`);
    }
  }
