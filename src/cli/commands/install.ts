...
export function registerInstallCommand(program: Command): void {
  ...
  install
    ...
    .option('--method <method>', 'Installation method (docker, homebrew, binary)')
...
function detectExistingFlink(): string | null {
  try {
    const output = execSync('flink --version 2>/dev/null', { encoding: 'utf-8' }).trim();
    const match = output.match(/Version:\s*([\d.]+)/);
    return match?.[1] ?? output;
  } catch {
    // Check Docker images
    try {
      const output = execSync('docker images --format "{{.Repository}}:{{.Tag}}" 2>/dev/null', {
        encoding: 'utf-8',
      }).trim();
      const flinkImage = output.split('\n').find(line => line.includes('flink'));
...
export async function runInstallFlink(opts: Record<string, unknown>): Promise<void> {
  ...
  console.log(`  ${pc.dim('Docker:')} ${info.hasDocker ? 'available' : 'not found'}`);
  if (info.os === 'macos') {
    console.log(`  ${pc.dim('Homebrew:')} ${info.hasHomebrew ? 'available' : 'not found'}`);
  }
  console.log('');
  ...
  if (opts.method) {
    ...
    if (info.hasDocker) {
      options.push({ value: 'docker', label: 'Docker (recommended)', hint: 'docker-compose.flink.yml' });
    }
    if (info.hasHomebrew) {
      options.push({ value: 'homebrew', label: 'Homebrew', hint: 'brew install apache-flink' });
    }
    options.push({ value: 'binary', label: 'Binary download', hint: 'Download from Apache mirrors' });

    if (options.length === 0) {
      console.error(pc.red('No installation methods available. Install Docker first.'));
      process.exitCode = 1;
      return;
    }
...
async function installViaDocker(flinkVersion: string): Promise<void> {
  ...
  console.log(`  ${pc.green('✓')} Created ${pc.dim('docker-compose.flink.yml')}`);
  ...
  try {
    ...
    spinner.stop(pc.yellow('Failed to pull image. You can pull it manually later.'));
    return;
  }
  ...
  try {
    const output = execSync(`docker run --rm flink:${flinkVersion} flink --version`, {
      encoding: 'utf-8',
  ...
  console.log('');
  console.log(`  Start the cluster: ${pc.dim('docker compose -f docker-compose.flink.yml up -d')}`);
...
async function installViaBinary(flinkVersion: string): Promise<void> {
  ...
  console.log(`  ${pc.dim('Extract:')} tar -xzf flink-${flinkVersion}.1-bin-scala_2.12.tgz`);
  console.log(`  ${pc.dim('Add to PATH:')} export PATH=$PATH:$(pwd)/flink-${flinkVersion}.1/bin`);
  console.log('');
}
