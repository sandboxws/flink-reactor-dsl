...
export function checkFlinkInstallation(): CheckResult {
  ...
  if (flinkOutput) {
    ...
    return { name: 'Apache Flink', status: 'pass', version, message: `v${version} (local)` };
  }

  // Check for Flink Docker image
  const dockerOutput = runCommand('docker images --format "{{.Repository}}:{{.Tag}}" | grep -i flink | head -1');
  if (dockerOutput) {
    return { name: 'Apache Flink', status: 'pass', message: `Docker: ${dockerOutput}` };
  }
...
export function checkKubectl(): CheckResult {
  const output = runCommand('kubectl version --client --short 2>/dev/null || kubectl version --client 2>/dev/null');
  if (!output) {
    return {
      name: 'kubectl',
      status: 'warn',
...
export function checkProjectConfig(projectDir: string): CheckResult {
  ...
  if (existsSync(configPath)) {
    return { name: 'Project config', status: 'pass', message: 'flink-reactor.config.ts found' };
  }
  return {
    name: 'Project config',
    status: 'warn',
...
export function discoverPipelines(projectDir: string): CheckResult {
  ...
  if (!existsSync(pipelinesDir)) {
    return { name: 'Pipelines', status: 'warn', message: 'No pipelines/ directory', hint: 'Run "flink-reactor generate pipeline <name>"' };
  }

  const count = countPipelines(pipelinesDir);
  if (count === 0) {
    return { name: 'Pipelines', status: 'warn', message: '0 pipelines discovered', hint: 'Run "flink-reactor generate pipeline <name>"' };
  }

  return { name: 'Pipelines', status: 'pass', message: `${count} pipeline${count !== 1 ? 's' : ''} discovered` };
}
...
function printResults(results: CheckResult[]): void {
  ...
  if (failures.length > 0) {
    ...
    if (warnings.length > 0) {
      console.log(pc.yellow(`All checks passed with ${warnings.length} warning(s).`));
    } else {
