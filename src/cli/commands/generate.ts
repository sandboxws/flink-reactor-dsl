...
export function registerGenerateCommand(program: Command): void {
  ...
  generate
    ...
    .option('-t, --template <template>', 'Pipeline template (blank, kafka, jdbc)', 'blank')
...
export function generateApp(name: string): void {
  if (!isMonorepo()) {
    console.error(pc.red('Error: "generate app" is only available in monorepo projects.'));
    console.log(pc.dim('  Hint: Run "flink-reactor new --template monorepo" to create a monorepo project.'));
    process.exitCode = 1;
    return;
  }
  ...
  writeIfNotExists(join(appDir, 'package.json'), JSON.stringify(pkg, null, 2) + '\n');
  writeIfNotExists(
...
export function generatePackage(name: string): void {
  if (!isMonorepo()) {
    console.error(pc.red('Error: "generate package" is only available in monorepo projects.'));
    console.log(pc.dim('  Hint: Run "flink-reactor new --template monorepo" to create a monorepo project.'));
    process.exitCode = 1;
    return;
  }
  ...
  writeIfNotExists(join(pkgDir, 'package.json'), JSON.stringify(pkg, null, 2) + '\n');
  writeIfNotExists(
