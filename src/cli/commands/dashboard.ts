...
import { resolveConfig, buildResolvedDashboardJson } from '../../core/config-resolver.js';
...
async function runDashboardExport(opts: {
  ...
  if (!config.environments || Object.keys(config.environments).length === 0) {
    ...
    console.error(pc.dim('Add an environments section to flink-reactor.config.ts.'));
    process.exitCode = 1;
    return;
  }
  ...
  console.log(pc.dim(`  COPY ${outputPath.replace(projectDir + '/', '')} /app/dashboard-config.json`));
