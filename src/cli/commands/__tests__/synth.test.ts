...
`;
  ...
  it('discovers and synthesizes a pipeline to dist/', async () => {
    ...
    // Check output files
    expect(existsSync(join(tempDir, 'dist', 'test-pipeline', 'pipeline.sql'))).toBe(true);
    expect(existsSync(join(tempDir, 'dist', 'test-pipeline', 'deployment.yaml'))).toBe(true);
    expect(existsSync(join(tempDir, 'dist', 'test-pipeline', 'configmap.yaml'))).toBe(true);

    // Validate SQL content
    const sql = readFileSync(join(tempDir, 'dist', 'test-pipeline', 'pipeline.sql'), 'utf-8');
    expect(sql).toContain('CREATE TABLE');
    expect(sql).toContain('INSERT INTO');

    // Validate YAML content
    const yaml = readFileSync(join(tempDir, 'dist', 'test-pipeline', 'deployment.yaml'), 'utf-8');
    expect(yaml).toContain('FlinkDeployment');
    expect(yaml).toContain('flink.apache.org/v1beta1');

    // Validate ConfigMap
    const configMap = readFileSync(join(tempDir, 'dist', 'test-pipeline', 'configmap.yaml'), 'utf-8');
  ...
  it('supports --outdir flag for custom output directory', async () => {
    ...
    expect(existsSync(join(tempDir, 'build', 'output', 'test-pipeline', 'pipeline.sql'))).toBe(true);
  });
});
