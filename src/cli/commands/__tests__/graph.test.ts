...
describe('graph command', () => {
  describe('ASCII output', () => {
    it('produces ASCII output for a simple pipeline', () => {
      const pipeline = createElement('Pipeline', { name: 'test-pipeline' },
        createElement('KafkaSink', { topic: 'output', name: 'output-sink' },
          createElement('Filter', { condition: 'amount > 100', name: 'filter' },
    ...
    it('includes node types and names in visualization', () => {
      const pipeline = createElement('Pipeline', { name: 'orders' },
        createElement('KafkaSource', {
          topic: 'orders',
          name: 'order-events',
  ...
  describe('DOT output', () => {
    it('produces valid DOT format', () => {
      const pipeline = createElement('Pipeline', { name: 'dot-test' },
        createElement('KafkaSink', { topic: 'output' },
          createElement('KafkaSource', {
            topic: 'input',
            schema: { fields: { id: 'BIGINT' }, metadataColumns: [] },
          })
    ...
    it('assigns colors by node kind', () => {
      const pipeline = createElement('Pipeline', { name: 'colored' },
        createElement('KafkaSink', { topic: 'output' },
          createElement('KafkaSource', {
            topic: 'input',
            schema: { fields: { id: 'BIGINT' }, metadataColumns: [] },
