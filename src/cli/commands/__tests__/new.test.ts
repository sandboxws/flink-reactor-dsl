...
describe('scaffoldProject', () => {
  ...
  describe('starter template', () => {
    ...
    it('generates pipeline with JSX content', () => {
      ...
      const pipeline = readFileSync(join(projectDir, 'pipelines', 'hello-world', 'index.tsx'), 'utf-8');
  ...
  describe('realtime-analytics template', () => {
    it('creates analytics pipeline with windowed aggregation', () => {
      scaffoldProject(projectDir, makeOptions({ template: 'realtime-analytics' }));
  ...
  describe('monorepo template', () => {
    it('creates workspace structure', () => {
      ...
      expect(existsSync(join(projectDir, 'packages', 'schemas', 'package.json'))).toBe(true);
      expect(existsSync(join(projectDir, 'packages', 'patterns', 'package.json'))).toBe(true);
      expect(existsSync(join(projectDir, 'apps', 'default-app', 'package.json'))).toBe(true);
      expect(existsSync(join(projectDir, 'apps', 'default-app', 'flink-reactor.config.ts'))).toBe(true);
    });
