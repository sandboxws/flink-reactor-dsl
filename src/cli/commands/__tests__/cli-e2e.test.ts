...
`,
    ...
    // Verify files
    expect(existsSync(join(tempDir, 'dist', 'orders', 'pipeline.sql'))).toBe(true);
    expect(existsSync(join(tempDir, 'dist', 'orders', 'deployment.yaml'))).toBe(true);
    expect(existsSync(join(tempDir, 'dist', 'orders', 'configmap.yaml'))).toBe(true);

    const sql = readFileSync(join(tempDir, 'dist', 'orders', 'pipeline.sql'), 'utf-8');
    expect(sql).toContain('raw-orders');
    expect(sql).toContain('processed-orders');
    expect(sql).toContain('total > 0');
    expect(sql).toContain('INSERT INTO');

    const yaml = readFileSync(join(tempDir, 'dist', 'orders', 'deployment.yaml'), 'utf-8');
    ...
    const pipelineNode = createElement('Pipeline', { name: 'orders', parallelism: 2 },
      createElement('KafkaSink', { topic: 'processed-orders' },
        createElement('Filter', { condition: 'total > 0' },
          createElement('KafkaSource', {
            topic: 'raw-orders',
            schema: { fields: { order_id: 'BIGINT', total: 'DOUBLE' }, metadataColumns: [] },
          })
        )
      )
    );
