...
import { SynthContext, type GraphEdge } from '../../core/synth-context.js';
...
export function registerGraphCommand(program: Command): void {
  program
    ...
    .action(async (opts: { pipeline?: string; format: string; open?: boolean }) => {
      await runGraph(opts);
    });
}
  ...
  for (const discovered of projectCtx.pipelines) {
    ...
    switch (format) {
      case 'dot':
        ...
        break;
      case 'svg':
        output = generateSvg(discovered.name, nodes, edges, projectDir, opts.open);
        break;
      default:
  ...
  function renderNode(nodeId: string, indent: number): void {
    ...
    for (const childId of children) {
      if (!visited.has(childId)) {
        ...
        lines.push(`${'  '.repeat(indent)}\u2514\u2500 ${pc.dim(`(ref) ${childLabel}`)}`);
      }
    }
  }
...
export function generateDot(
  ...
  for (const node of nodes) {
    ...
    lines.push(`  "${node.id}" [label="${label}", fillcolor="${color}", style="rounded,filled", fontcolor="white"];`);
  }
...
function generateSvg(
  ...
  try {
    ...
    console.error(pc.red('SVG generation requires graphviz. Install with: brew install graphviz'));
