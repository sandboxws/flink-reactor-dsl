...
function resolveFilePath(pathArg: string, projectDir: string): string | null {
  ...
  if (pathArg.includes('/') || pathArg.includes('\\') || pathArg.endsWith('.tsx')) {
...
function printSchemas(schemas: IntrospectedSchema[], pipelineName?: string): void {
  ...
  // Sources
  if (sources.length > 0) {
    console.log(`  ${pc.bold(pc.green('■'))} ${pc.bold('Sources (Input Schemas)')}`);
    console.log('');
  ...
  // Sinks
  if (sinks.length > 0) {
    console.log(`  ${pc.bold(pc.magenta('■'))} ${pc.bold('Sinks (Output Schemas)')}`);
    console.log('');
    for (const sink of sinks) {
      if (sink.columns.length === 0) {
        console.log(`  ${pc.dim('─')} ${pc.bold(sink.nameHint)} ${pc.dim(`(${sink.component})`)}`);
...
function printSchemaTable(schema: IntrospectedSchema): void {
  ...
  // Header
  console.log(`  ${pc.dim('─')} ${pc.bold(nameHint)} ${pc.dim(`(${component})`)}`);
...
function printTableWithConstraints(
  ...
  // Top border
  console.log(`  ${pc.dim('┌' + '─'.repeat(fw) + '┬' + '─'.repeat(tw) + '┬' + '─'.repeat(cw) + '┐')}`);
