...
const IOT_DEVICES: IoTDevice[] = [
  { device_id: 'dev-001', device_name: 'Warehouse-TempSensor-A1', device_type: 'temperature', location: 'warehouse-a', firmware: '2.1.0', status: 'active' },
  { device_id: 'dev-002', device_name: 'Warehouse-HumiditySensor-A2', device_type: 'humidity', location: 'warehouse-a', firmware: '2.1.0', status: 'active' },
  { device_id: 'dev-003', device_name: 'Factory-PressureSensor-B1', device_type: 'pressure', location: 'factory-b', firmware: '2.0.3', status: 'active' },
  { device_id: 'dev-004', device_name: 'Factory-VibrationSensor-B2', device_type: 'vibration', location: 'factory-b', firmware: '2.0.3', status: 'active' },
  { device_id: 'dev-005', device_name: 'Office-TempSensor-C1', device_type: 'temperature', location: 'office-c', firmware: '2.1.0', status: 'active' },
  { device_id: 'dev-006', device_name: 'Office-HumiditySensor-C2', device_type: 'humidity', location: 'office-c', firmware: '2.1.0', status: 'active' },
  { device_id: 'dev-007', device_name: 'Dock-TempSensor-D1', device_type: 'temperature', location: 'loading-dock-d', firmware: '1.9.5', status: 'active' },
  { device_id: 'dev-008', device_name: 'Dock-PressureSensor-D2', device_type: 'pressure', location: 'loading-dock-d', firmware: '1.9.5', status: 'active' },
  { device_id: 'dev-009', device_name: 'Warehouse-VibrationSensor-A3', device_type: 'vibration', location: 'warehouse-a', firmware: '2.0.3', status: 'active' },
  { device_id: 'dev-010', device_name: 'Factory-TempSensor-B3', device_type: 'temperature', location: 'factory-b', firmware: '2.1.0', status: 'active' },
];
  ...
  if (opts.mode === 'batch') {
    ...
    if (domain === 'ecommerce' || domain === 'all') {
      ...
      if (domain === 'all') {
        ...
        await publishEcommerceContinuous(composeFile, topic, intervalMs, opts.signal);
      }
    } else if (domain === 'iot') {
      await publishIoTContinuous(composeFile, 3_000, opts.signal);
    }
...
async function publishEcommerceBatch(composeFile: string | undefined, topic: string): Promise<void> {
  const messages: string[] = [];

  // 10 inserts
  for (const product of SEED_PRODUCTS) {
    messages.push(
      JSON.stringify(createDebeziumEnvelope('c', null, product, 'inventory', 'products')),
    );
  }
  ...
  for (const product of SEED_PRODUCTS) {
    ...
    messages.push(JSON.stringify(createDebeziumEnvelope('u', before1, after1, 'inventory', 'products')));

    const before2 = { ...after1 };
    const after2 = { ...after1, quantity: Math.max(0, after1.quantity + randomInt(-20, 30)) };
    messages.push(JSON.stringify(createDebeziumEnvelope('u', before2, after2, 'inventory', 'products')));
  }

  // 2 deletes (last two products)
  for (const product of SEED_PRODUCTS.slice(-2)) {
    messages.push(
      JSON.stringify(createDebeziumEnvelope('d', product, null, 'inventory', 'products')),
    );
  }
...
async function publishEcommerceContinuous(
  ...
  while (!signal?.aborted) {
    ...
    const message = JSON.stringify(createDebeziumEnvelope('u', before, after, 'inventory', 'products'));
    try {
...
async function publishIoTBatch(composeFile: string | undefined): Promise<void> {
  ...
  for (const device of IOT_DEVICES) {
    deviceMessages.push(
      JSON.stringify(createDebeziumEnvelope('c', null, device, 'iot', 'devices')),
    );
  }
  publishToKafka(composeFile, deviceTopic, deviceMessages);
  console.log(`  ${pc.green('✓')} Published ${deviceMessages.length} device creates to ${pc.dim(deviceTopic)}`);
  ...
  for (const device of IOT_DEVICES) {
    for (let i = 0; i < 2; i++) {
      ...
      telemetryMessages.push(
        JSON.stringify(createDebeziumEnvelope('c', null, reading, 'iot', 'telemetry')),
      );
    }
  }
  publishToKafka(composeFile, telemetryTopic, telemetryMessages);
  console.log(`  ${pc.green('✓')} Published ${telemetryMessages.length} telemetry readings to ${pc.dim(telemetryTopic)}`);
  ...
  for (const device of IOT_DEVICES) {
    ...
    updateMessages.push(
      JSON.stringify(createDebeziumEnvelope('u', before, after, 'iot', 'devices')),
    );
  }
  ...
  for (const device of IOT_DEVICES.slice(-2)) {
    ...
    updateMessages.push(
      JSON.stringify(createDebeziumEnvelope('u', updated, decommissioned, 'iot', 'devices')),
    );
  }
  publishToKafka(composeFile, deviceTopic, updateMessages);
  console.log(`  ${pc.green('✓')} Published ${updateMessages.length} device updates to ${pc.dim(deviceTopic)}`);
}
...
async function publishIoTContinuous(
  ...
  while (!signal?.aborted) {
    ...
    const reading = generateReading(device);
    const message = JSON.stringify(createDebeziumEnvelope('c', null, reading, 'iot', 'telemetry'));
